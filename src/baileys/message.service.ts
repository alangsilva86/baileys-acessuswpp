...
function extractMessageText(message: WAMessage): string | null {
  ...
  if (content.pollCreationMessage?.name) {
    return content.pollCreationMessage.name;
  }
  if (content.pollCreationMessageV2?.name) {
    return content.pollCreationMessageV2.name;
  }
  if (content.pollCreationMessageV3?.name) {
    return content.pollCreationMessageV3.name;
  }
  return null;
}
