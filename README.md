...
As etapas de jornada descritas na revisão de UX (`docs/dashboard-ux-review.md`) estão implementadas em **duas UIs**:

- **UI canônica (React SPA)**: código em `apps/web`, build via Vite para `dist/web` (servida quando existe).
- **UI legada (fallback)**: HTML em `public/index.html` + módulos ES6 em `public/js`.

### UI canônica (React SPA)

- **Etapa 1 — Boot / API key / real-time**: `apps/web/src/main.tsx`, `apps/web/src/lib/api.ts` e SSE em `apps/web/src/features/dashboard/hooks/useInstanceMetrics.ts` + `apps/web/src/features/dashboard/components/MessageMonitorDrawer.tsx`.
- **Etapa 2 — Seleção e gerenciamento de instâncias**: `apps/web/src/features/dashboard/components/SidebarContainer.tsx` + `apps/web/src/features/dashboard/hooks/useInstances.ts`.
- **Etapa 3 — Contexto da instância e notas**: `apps/web/src/features/dashboard/components/DashboardMain.tsx` (Notas) + `PATCH /instances/:iid`.
- **Etapa 4 — Métricas e export por período**: `apps/web/src/features/dashboard/hooks/useInstanceMetrics.ts` + UI em `apps/web/src/features/dashboard/components/DashboardMain.tsx`.
- **Etapa 5 — Sessão, QR e ações críticas**: `apps/web/src/features/dashboard/components/DashboardMain.tsx` (QR, logout, proxy, risco).
- **Etapa 6 — Envio rápido**: `apps/web/src/features/dashboard/components/DashboardMain.tsx` (modal) + `POST /instances/:iid/send-quick`.
...
- **Etapa 4 — Métricas e timeline**: `public/js/metrics.js` controla KPIs e sincronização da instância ativa.
- **Etapa 5 — Ações de sessão e QR Code**: `public/js/sessionActions.js` agrega os handlers de logout/wipe/pair, modais e feedback do QR.
- **Etapa 6 — Envio rápido**: `public/js/quickSend.js` concentra validações e o envio de mensagens rápidas.
- **Logs recentes**: `public/js/logs.js` renderiza eventos e entregas para contexto operacional.
...
- **Autenticação**: o Pipedrive injeta um token (JWT) na URL do painel (`token=` ou `jwt=`). Se o token não vier, o painel mostrará “Token ausente na URL do painel do Pipedrive.” — ajuste a configuração do painel no Developer Hub para enviar o token.
